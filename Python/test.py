# test.py
"""
This driver program reads input files (generated by your data generator) for three case types:
AVERAGE, BEST, and WORST. For each case type and for each input size (1000, 5000, 10000, 30000),
it runs both the FF and FFD algorithms, measures their execution times in milliseconds, and records
the number of bins used. The results are appended to a CSV file named "python_results.csv" in the same folder.
The CSV output for each case type is formatted as follows:
...
def process_file(filename, bin_capacity):
    ...
    the FF and FFD algorithms while timing them in milliseconds.
    ...
    Returns:
        list: A list containing the filename, FF time (ms), FFD time (ms), 
    ...
    # Measure FF execution time in milliseconds.
    start_time = time.time()
    bins_ff = first_fit(numbers, bin_capacity)
    time_ff = (time.time() - start_time) * 1000  # convert seconds to milliseconds

    # Measure FFD execution time in milliseconds.
    start_time = time.time()
    bins_ffd = first_fit_decreasing(numbers, bin_capacity)
    time_ffd = (time.time() - start_time) * 1000  # convert seconds to milliseconds
...
def main():
    ...
    # (On some systems, use "%-m/%-d/%Y %I:%M:%S %p" for non-padded month/day; here we use padded format for portability.)
